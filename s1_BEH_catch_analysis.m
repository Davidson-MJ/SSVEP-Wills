...
%$
job.calcChunkaiShuffledCatchOnsetBPProb=0; % previous version, incorrect shuff procedure.

job.calcNEWShuffledCatchOnsetBPProb=0; %new ver MD # 04/18.

job.calcMissedcatchesbyTargperppant=0;
...
%             tmp4=datais(ippant).mProbTraceforFourtargetcatch;
    ...
    % note we now have two types.... 
%chunkai's shuff type (no replacement):
...
try
    ...
    %%
        ...
        anovadata = nan(2,48, 601); %for comparing each time point.
...
if job.plotCatchtracesbynumTargets==1
    ...
    for onsetoroffset=1:2
        ...
        for ippant=1:nppants
            ...
            try    stack(ippant,4,:)=tmp4; %if we have data for 4 target catch disappearance.
            catch
            end
    ...
    for onsetoroffset = 1:2
        ...
        anovadata = zeros(length(goodppants),4, 601); %for comparing each time point.
        ...
        tmp= squeeze(nanmean(acrossall_mShuffledCatch(goodppants,:,onsetoroffset,:,:),2));
        %mean across shuffs( 26 ppants).
        tmp= squeeze(nanmean(tmp,2));

        mShuff = squeeze(nanmean(tmp,1)); %m across ppants.
               
        
%% % %% % %% % %% %  What type of error bar to plot? CI at the moment.
...
%         
    ...
    usingErrbar = 'SEM';
               ...
               newX= tmpX - repmat(mX, [1,size(tmpX,2)]) + repmat(mmX, [size(tmpX,1), size(tmpX,2)]);
               
               stErrShuff= nanstd(newX)/ sqrt(size(newX,1));
        ...
        %plot shuffled: 
        %%
        subplot(1,2,onsetoroffset)
        if onsetoroffset==1
        shf=shadedErrorBar(xtONSET, mShuff, stErrShuff, 'm',1);
        xlim([-1 2])
        eventis= 'onset' ;
        
        
        else
        shf=shadedErrorBar(xtOFFSET, mShuff, stErrShuff, 'm',1);
        xlim([-1 2])
        eventis='offset';
        end
        %%
...
if useSEMorCI==1
              tmpX=dt;
               ...
               stErrM= nanstd(newX)/ sqrt(size(newX,1));
...
%         
            ...
            %width of shaded region to plot
            stErrM(1,ip)= CI(2)-median(CI); % half for plotting
            Shuffupper(1,ip)= CI(2);
            Shufflower(1,ip)= CI(1);
        end
            ...
            if onsetoroffset==1
            pl =shadedErrorBar(xtONSET, nanmean(dt,1), stErrM, colis,1);
            pl.mainLine.Color = [0 .5 0];
            pl.patch.FaceColor = [0 .5 0];
            else
            pl =shadedErrorBar(xtOFFSET, nanmean(dt,1), stErrM, 'r',1);
            end
            pl_leg(itarg)=pl.patch;
            hold on
            
            
            anovadata(:,itarg+1,:)= dt;
            p=[];
            end
...
%         title(['Target ' eventis], 'fontsize', 15)
        
        % plot(
        
        %%
        
%         lg=legend([pl_leg(1), pl_leg(2), pl_leg(3),  shf.patch, sigt], {'1 Target removed', '2 Targets removed', '3 Targets removed',   'Shuffled Likelihood', ['sig at ' sprintf('%.2f', placement) 'seconds']});
%         lg=legend([pl_leg(1), pl_leg(2), pl_leg(3),  shf.patch], {'1 Target removed', '2 Targets removed', '3 Targets removed',   'Shuffled Likelihood'});
%         set(lg, 'Location', 'SouthEast', 'fontsize', 1.5*15)
...
%         st.FontWeight='bold';
    ...
    print('-dpng', ['Timecourse of each CatchDisap BP, across all ' usingErrbar ' n=13'])
end
%%
